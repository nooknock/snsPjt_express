...
nunjucks.configure('views',{express:app, watch:true}); // watch:true이면 HTML 파일이 변경될 때 템플릿 엔진을 다시 렌더링

//sync 메서드를 사용해 서버 실행시 MySQL과 연동
sequelize.sync({force:false}) //true로 설정하면 서버 실행시마다 테이블을 재생성, table을 잘 못 만든 경우에 true로 설정
    .then(
        ()=>{
